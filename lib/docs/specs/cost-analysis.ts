...
export const costAnalysisGroup: ResourceGroup = {
  id: 'cost-analysis',
  label: 'Cost Analysis',
  description: 'Access Machine Hour Rate (MHR), Labour Hour Rate (LHR), tooling, and packaging cost engines.',
  icon: 'Calculator',
  endpoints: [
    {
      id: 'list-mhr',
    ...
    },
    {
      ...
      description: 'Returns all Labour Hour Rate (LHR) records. LHR represents the hourly cost of different labor grades including wages, benefits, and overheads.',
      parameters: [
        { name: 'page', in: 'query', type: 'integer', required: false, description: 'Page number', default: 1 },
        { name: 'grade', in: 'query', type: 'string', required: false, description: 'Filter by labor grade' },
      ],
      responses: [
        { statusCode: 200, description: 'LHR records retrieved', example: { success: true, data: { records: [{ id: 'lhr-uuid', grade: 'Skilled', hourlyRate: 280, currency: 'INR' }], total: 12 } } },
      ],
      errors: [{ statusCode: 401, code: 'UNAUTHORIZED', description: 'Invalid token' }],
      examples: [
    ...
    },
    {
      id: 'calculate-lhr',
      method: 'POST',
      path: '/v1/api/lhr/calculate',
      summary: 'Calculate labor cost',
      description: 'Calculates the labor cost for an operation based on standard time and a specific Labour Hour Rate.',
      parameters: [],
      requestBody: {
        description: 'Calculation inputs',
        required: true,
        schema: {
          ...
          type: 'object',
          properties: [
            { name: 'lhrId', type: 'string', required: true, description: 'Labour Hour Rate UUID', format: 'uuid' },
            { name: 'standardTimeSeconds', type: 'number', required: true, description: 'Standard time per unit in seconds', example: 120 },
            { name: 'quantity', type: 'integer', required: false, description: 'Number of parts', default: 1 },
          ],
        },
        example: { lhrId: 'lhr-uuid', standardTimeSeconds: 120, quantity: 1000 },
      },
      responses: [
      ...
      examples: [
        { language: 'curl', label: 'cURL', code: `curl -X POST ${BASE_URL}/v1/api/lhr/calculate \\\n  -H "Authorization: Bearer {token}" \\\n  -H "Content-Type: application/json" \\\n  -d '{"lhrId":"lhr-uuid","standardTimeSeconds":120,"quantity":1000}'` },
        { language: 'javascript', label: 'JavaScript', code: `const response = await fetch('${BASE_URL}/v1/api/lhr/calculate', {\n  method: 'POST',\n  headers: { Authorization: \`Bearer \${token}\`, 'Content-Type': 'application/json' },\n  body: JSON.stringify({ lhrId: 'lhr-uuid', standardTimeSeconds: 120, quantity: 1000 }),\n});` },
        { language: 'python', label: 'Python', code: `response = requests.post(\n    '${BASE_URL}/v1/api/lhr/calculate',\n    headers={'Authorization': f'Bearer {token}'},\n    json={'lhrId': 'lhr-uuid', 'standardTimeSeconds': 120, 'quantity': 1000},\n)` },
        { language: 'go', label: 'Go', code: `payload := \`{"lhrId":"lhr-uuid","standardTimeSeconds":120,"quantity":1000}\`\nreq, _ := http.NewRequest("POST", "${BASE_URL}/v1/api/lhr/calculate", strings.NewReader(payload))\nreq.Header.Set("Authorization", "Bearer "+token)\nreq.Header.Set("Content-Type", "application/json")` },
      ],
      requiresAuth: true,
    },
    {
