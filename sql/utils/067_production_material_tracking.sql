-- ============================================================================
-- MIGRATION 067: Production Material Tracking & Monitoring System
-- ============================================================================
-- Adds comprehensive material tracking and real-time monitoring capabilities
-- for the IntegratedMonitoringBOM component
-- ============================================================================

-- ============================================================================
-- TABLE: production_lot_materials
-- ============================================================================
-- Enhanced material tracking for production lots with real-time status
...
-- ============================================================================
-- TABLE: material_tracking_history
-- ============================================================================
-- Complete audit trail for material movements and status changes
...
-- ============================================================================
-- TABLE: production_material_alerts
-- ============================================================================
-- Real-time alerts for material and production issues
...
-- ============================================================================
-- TABLE: production_monitoring_metrics
-- ============================================================================
-- Real-time production metrics for dashboard and analytics
...
-- ============================================================================
-- FUNCTIONS FOR AUTOMATED TRACKING AND ALERTS
-- ============================================================================

-- Function to automatically create tracking history entry
CREATE OR REPLACE FUNCTION create_material_tracking_entry()
RETURNS TRIGGER AS $$
DECLARE
    action_type VARCHAR(50);
    quantity_changed DECIMAL(12, 3);
    previous_qty DECIMAL(12, 3);
BEGIN
    -- Determine action type based on what changed
    IF NEW.ordered_quantity != COALESCE(OLD.ordered_quantity, 0) THEN
        action_type := 'ordered';
        quantity_changed := NEW.ordered_quantity - COALESCE(OLD.ordered_quantity, 0);
        previous_qty := COALESCE(OLD.ordered_quantity, 0);
    ELSIF NEW.received_quantity != COALESCE(OLD.received_quantity, 0) THEN
        action_type := 'received';
        quantity_changed := NEW.received_quantity - COALESCE(OLD.received_quantity, 0);
        previous_qty := COALESCE(OLD.received_quantity, 0);
    ELSIF NEW.inspected_quantity != COALESCE(OLD.inspected_quantity, 0) THEN
        action_type := 'inspected';
        quantity_changed := NEW.inspected_quantity - COALESCE(OLD.inspected_quantity, 0);
        previous_qty := COALESCE(OLD.inspected_quantity, 0);
    ELSIF NEW.approved_quantity != COALESCE(OLD.approved_quantity, 0) THEN
        action_type := 'approved';
        quantity_changed := NEW.approved_quantity - COALESCE(OLD.approved_quantity, 0);
        previous_qty := COALESCE(OLD.approved_quantity, 0);
    ELSIF NEW.rejected_quantity != COALESCE(OLD.rejected_quantity, 0) THEN
        action_type := 'rejected';
        quantity_changed := NEW.rejected_quantity - COALESCE(OLD.rejected_quantity, 0);
        previous_qty := COALESCE(OLD.rejected_quantity, 0);
    ELSIF NEW.consumed_quantity != COALESCE(OLD.consumed_quantity, 0) THEN
        action_type := 'consumed';
        quantity_changed := NEW.consumed_quantity - COALESCE(OLD.consumed_quantity, 0);
        previous_qty := COALESCE(OLD.consumed_quantity, 0);
    END IF;
    
    -- Create tracking entry if there was a quantity change
    IF action_type IS NOT NULL AND quantity_changed != 0 THEN
        INSERT INTO material_tracking_history (
            production_lot_material_id,
            action,
            quantity,
            previous_quantity,
            new_quantity,
            ...
            previous_qty,
            CASE 
                WHEN action_type = 'ordered' THEN NEW.ordered_quantity
                WHEN action_type = 'received' THEN NEW.received_quantity
                WHEN action_type = 'inspected' THEN NEW.inspected_quantity
                WHEN action_type = 'approved' THEN NEW.approved_quantity
                WHEN action_type = 'rejected' THEN NEW.rejected_quantity
                WHEN action_type = 'consumed' THEN NEW.consumed_quantity
            END,
            NEW.batch_number,
            CASE 
                WHEN action_type = 'consumed' THEN 'Material consumed in production'
                WHEN action_type = 'approved' THEN 'Material approved after inspection'
                WHEN action_type = 'received' THEN 'Material received from vendor'
                ELSE 'Material status updated'
            END,
            auth.uid(),
            'System Auto-Update'
        );
...
-- Trigger for automatic tracking
CREATE TRIGGER trigger_create_material_tracking
    AFTER UPDATE ON production_lot_materials
    FOR EACH ROW
    EXECUTE FUNCTION create_material_tracking_entry();

-- Function to check for material shortages and create alerts
CREATE OR REPLACE FUNCTION check_material_alerts()
RETURNS TRIGGER AS $$
BEGIN
    -- Check for material shortage alerts
    IF NEW.approved_quantity < NEW.required_quantity * 0.8 AND NEW.material_status IN ('approved', 'in_use') THEN
        INSERT INTO production_material_alerts (
            production_lot_id,
            production_lot_material_id,
            alert_type,
            severity,
            ...
            'shortage',
            CASE 
                WHEN NEW.approved_quantity < NEW.required_quantity * 0.5 THEN 'critical'
                WHEN NEW.approved_quantity < NEW.required_quantity * 0.7 THEN 'high'
                ELSE 'medium'
            END,
            'system',
            'Material Shortage Alert',
            'Material ' || (SELECT part_name FROM bom_items WHERE id = NEW.bom_item_id) || ' is running low',
            'Production may be delayed if material is not replenished soon',
            'Order additional material or adjust production schedule'
        )
        ON CONFLICT DO NOTHING;
    END IF;
    
    -- Check for reorder point alerts
    IF NEW.approved_quantity <= NEW.reorder_point AND NEW.reorder_point > 0 THEN
        INSERT INTO production_material_alerts (
            production_lot_id,
            production_lot_material_id,
            alert_type,
            ...
            'Material stock has reached reorder point',
            'Initiate reorder process to maintain adequate inventory'
        )
        ON CONFLICT DO NOTHING;
    END IF;
...
-- Trigger for material alerts
CREATE TRIGGER trigger_check_material_alerts
    AFTER UPDATE ON production_lot_materials
    FOR EACH ROW
    EXECUTE FUNCTION check_material_alerts();

-- ============================================================================
-- UTILITY FUNCTIONS FOR DASHBOARD DATA
-- ============================================================================

-- Function to get production lot material readiness percentage
