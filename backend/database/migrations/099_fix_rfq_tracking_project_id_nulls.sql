-- Migration 099: Fix RFQ Tracking Project ID Null Values
...
-- Option 1: Try to populate project_id from related RFQ data
UPDATE rfq_tracking 
SET project_id = rfqs.project_id
FROM rfqs
WHERE rfq_tracking.rfq_id = rfqs.id 
AND rfq_tracking.project_id IS NULL
AND rfqs.project_id IS NOT NULL;
...
BEGIN
    -- Check for RFQ tracking records referencing non-existent RFQs
    SELECT COUNT(*) INTO orphaned_rfqs 
    FROM rfq_tracking rt 
    WHERE rt.rfq_id IS NOT NULL 
    AND NOT EXISTS (SELECT 1 FROM rfqs r WHERE r.id = rt.rfq_id);
    
    IF orphaned_rfqs > 0 THEN
        RAISE NOTICE 'Found % RFQ tracking records referencing non-existent RFQs', orphaned_rfqs;
    END IF;
