-- ============================================================================
-- Calculator: Machining - End Milling and Sawing
-- Category:   Process
-- Fields:     28 (8 W/C, 8 process, 12 calculated)
-- Key difference from Face Milling: uses Feed/rev (not Feed/tooth) in time formula
--   Machining Time = ((Length + 10 + Cutter Dia/2) / (Feed/rev × No Teeth × RPM))
--                   × No of Passes × (Total Depth / Depth per Pass)
...
BEGIN
  INSERT INTO calculators (
    ...
    v_user_id,
    'Machining - End Milling and Sawing',
    'process',
    'single',
    false,
    false,
    'End milling / sawing – cutter RPM, lateral and depth passes, machining time using feed/rev, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Width',              'Workpiece Width (mm)',      'number', 'mm',       NULL, 10, false, NULL),
    (v_calc_id, 'Length',             'Workpiece Length (mm)',     'number', 'mm',       NULL, 11, false, NULL),
    ...
    (v_calc_id, 'Feed per Rev',       'Feed/rev (mm/rev)',         'number', 'mm/rev',   NULL, 15, false, NULL),
    ...
    (v_calc_id, 'No of Passes',       'No. of Lateral Passes',     'calculated', NULL,  NULL, 18, false, NULL),
    (v_calc_id, 'Machining Time Min', 'Machining Time (min)',      'calculated', 'min', NULL, 19, false, NULL),
    (v_calc_id, 'Machining Time',     'Machining Time (s)',        'calculated', 's',   NULL, 20, false, NULL),
    (v_calc_id, 'Time per Use',       'Time per Use (s)',          'calculated', 's',   NULL, 21, false, NULL),
    (v_calc_id, 'Total Time',         'Total Time (s)',            'calculated', 's',   NULL, 22, false, NULL),
    (v_calc_id, 'Tool Cost per Part', 'Tool Cost/Part (INR)',      'calculated', 'INR', NULL, 23, false, NULL),
    (v_calc_id, 'Machine Cost',       'Machine Cost (INR)',        'calculated', 'INR', NULL, 24, false, NULL),
    (v_calc_id, 'Labour Cost',        'Labour Cost (INR)',         'calculated', 'INR', NULL, 25, false, NULL),
    (v_calc_id, 'Process Cost',       'Process Cost (INR)',        'calculated', 'INR', NULL, 26, false, NULL),
    (v_calc_id, 'Setup Cost',         'Setup Cost (INR)',          'calculated', 'INR', NULL, 27, false, NULL),
    (v_calc_id, 'Total Process Cost', 'Total Process Cost (INR)', 'calculated', 'INR', NULL, 28, false, NULL);
  ...
  UPDATE calculator_fields SET default_value = '({Width} - {Cutter Diameter} / 2) / ({Cutter Diameter} - 2)'
  WHERE calculator_id = v_calc_id AND field_name = 'No of Passes';

  UPDATE calculator_fields SET default_value = '(({Length} + 10 + {Cutter Diameter} / 2) / ({Feed per Rev} * {No of Teeth} * {Spindle RPM})) * {No of Passes} * ({Total Depth} / {Depth per Pass})'
  ...
  RAISE NOTICE 'Done — Machining End Milling and Sawing, ID: %', v_calc_id;
END $$;
