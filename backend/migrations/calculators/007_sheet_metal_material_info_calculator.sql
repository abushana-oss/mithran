-- ============================================================================
-- Calculator: Sheet Metal - Material Information
-- Category:   Material
-- Fields:     26 (3 Text, 4 DB Lookup, 9 Number, 10 Calculated)
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Sheet metal raw material cost – parts per sheet, weight, utilisation, net material cost'
  )
  ...
  VALUES
    ...
    -- DB Lookups (material properties)
    ...
    (v_calc_id, 'Net Weight Per Part',  'Net weight per part (g)',  'number',         'g',     NULL,            11, false, NULL),
    (v_calc_id, 'Area',                 'Area (mm²)',               'number',         'mm²',   NULL,            12, false, NULL),

    -- Sheet parameters
    (v_calc_id, 'Sheet Width',          'Sheet Width (mm)',         'number',         'mm',    NULL,            15, false, '1250'),
    (v_calc_id, 'Sheet Length',         'Sheet Length (mm)',        'number',         'mm',    NULL,            16, false, '2500'),
    (v_calc_id, 'Edge Allowance',       'Edge Allowance (mm)',      'number',         'mm',    NULL,            17, false, '2'),
    (v_calc_id, 'Scrap Recovery',       'Scrap Recovery %',        'number',         '%',     NULL,            22, false, '90'),
    ...
    (v_calc_id, 'Parts Per Sheet',      'Parts per Sheet',          'calculated',     NULL,    NULL,            18, false, NULL),
    (v_calc_id, 'Sheet Weight',         'Sheet Weight (g)',         'calculated',     'g',     NULL,            19, false, NULL),
    (v_calc_id, 'Gross Weight Per Part','Gross weight per part (g)','calculated',     'g',     NULL,            20, false, NULL),
    (v_calc_id, 'Scrap Weight Per Part','Scrap weight per part (g)','calculated',     'g',     NULL,            21, false, NULL),
    (v_calc_id, 'Utilisation',          'Utilisation %',           'calculated',     '%',     NULL,            23, false, NULL),
    (v_calc_id, 'Gross Material Cost',  'Gross Material cost ($)',  'calculated',     '$',     NULL,            24, false, NULL),
    (v_calc_id, 'Scrap Rec Cost',       'Scrap Rec Cost ($)',       'calculated',     '$',     NULL,            25, false, NULL),
    (v_calc_id, 'Net Material Cost',    'Net Material cost ($)',    'calculated',     '$',     NULL,            26, false, NULL);
  ...
  RAISE NOTICE 'Done — Sheet Metal - Material Information, ID: %', v_calc_id;
END $$;
