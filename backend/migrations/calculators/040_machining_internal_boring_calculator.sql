-- ============================================================================
-- Calculator: Machining - Internal Boring
-- Category:   Process
-- Fields:     25 (8 W/C, 6 process, 11 calculated)
-- Formulas:
--   Spindle RPM    = 1000 × Cutting Speed / (π × Initial Diameter)
--   No of Passes   = CEIL((Final Dia − Initial Dia) / (2 × Depth of Cut))
--   Machining Time = (π × Initial Dia × (Length + 4)) / (1000 × Vc × Feed/rev) × No of Passes
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Internal boring – spindle RPM based on initial bore, passes to open to final diameter, machining time, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Initial Diameter',   'Initial Bore Diameter (mm)',     'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Final Diameter',     'Final Bore Diameter (mm)',       'number', 'mm',     NULL, 10, false, NULL),
    (v_calc_id, 'Length',             'Boring Length (mm)',             'number', 'mm',     NULL, 11, false, NULL),
    (v_calc_id, 'Depth of Cut',       'Depth of Cut per Pass (mm)',     'number', 'mm',     NULL, 12, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '1000 * {Cutting Speed} / (3.14159265 * {Initial Diameter})'
  WHERE calculator_id = v_calc_id AND field_name = 'Spindle RPM';

  UPDATE calculator_fields SET default_value = 'CEIL(({Final Diameter} - {Initial Diameter}) / (2 * {Depth of Cut}))'
  WHERE calculator_id = v_calc_id AND field_name = 'No of Passes';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Initial Diameter} * ({Length} + 4)) / (1000 * {Cutting Speed} * {Feed per Rev}) * {No of Passes}'
  ...
  RAISE NOTICE 'Done — Machining Internal Boring, ID: %', v_calc_id;
END $$;
