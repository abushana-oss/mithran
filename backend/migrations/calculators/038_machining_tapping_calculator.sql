...
--   Note: Feed/rev = thread pitch for tapping
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Tapping – spindle RPM, machining time with 4mm approach, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Tap Diameter',       'Tap Diameter (mm)',       'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Length',             'Tapping Length (mm)',     'number', 'mm',     NULL, 10, false, NULL),
    (v_calc_id, 'Cutting Speed',      'Cutting Speed (m/min)',   'number', 'm/min',  NULL, 11, false, NULL),
    (v_calc_id, 'Feed per Rev',       'Feed/rev / Pitch (mm)',   'number', 'mm/rev', NULL, 12, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '1000 * {Cutting Speed} / (3.14159265 * {Tap Diameter})'
  WHERE calculator_id = v_calc_id AND field_name = 'Spindle RPM';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Tap Diameter} * ({Length} + 4)) / (1000 * {Cutting Speed} * {Feed per Rev})'
  ...
  RAISE NOTICE 'Done — Machining Tapping, ID: %', v_calc_id;
END $$;
