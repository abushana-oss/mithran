-- ============================================================================
-- Calculator: Machining - Turning
-- Category:   Process
-- Fields:     26 (8 W/C, 6 process, 12 calculated)
-- Formulas:
--   Spindle RPM  = 1000 × Cutting Speed / (π × Initial Diameter)
--   No of Passes = (Initial Dia − Final Dia) / (2 × Depth of Cut)
--   Machining Time = (π × Initial Dia × (Length + 10)) / (1000 × Vc × Feed/rev) × Passes
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Turning operation – spindle RPM, no. of passes from diameter reduction, machining time, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Initial Diameter',   'Initial Diameter (mm)',          'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Final Diameter',     'Final Diameter (mm)',            'number', 'mm',     NULL, 10, false, NULL),
    (v_calc_id, 'Length',             'Length of Cut (mm)',             'number', 'mm',     NULL, 11, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '({Initial Diameter} - {Final Diameter}) / (2 * {Depth of Cut})'
  WHERE calculator_id = v_calc_id AND field_name = 'No of Passes';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Initial Diameter} * ({Length} + 10)) / (1000 * {Cutting Speed} * {Feed per Rev}) * {No of Passes}'
  ...
  RAISE NOTICE 'Done — Machining Turning, ID: %', v_calc_id;
END $$;
