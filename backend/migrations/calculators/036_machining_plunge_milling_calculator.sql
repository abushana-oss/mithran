-- ============================================================================
-- Calculator: Machining - Plunge Milling
-- Category:   Process
-- Fields:     28 (8 W/C, 7 process, 13 calculated)
-- Formulas:
--   Length of Cut (fixed) = 3.7 × π × 10 ≈ 116.24 mm
--   Spindle RPM           = 1000 × Cutting Speed / (π × Cutter Diameter)
--   No of Depth Passes    = CEIL(Total Depth / Depth per Pass)
--   No of Width Passes    = CEIL(Width / Cutter Diameter)
--   Machining Time        = ((Length of Cut + 10 + Cutter Dia) / (Feed/tooth × No Teeth × RPM))
--                           × No of Depth Passes × No of Width Passes
...
BEGIN
  INSERT INTO calculators (
    ...
    v_user_id,
    'Machining - Plunge Milling',
    'process',
    'single',
    false,
    false,
    'Plunge milling – fixed geometry length of cut (3.7×π×10), depth and width passes, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Width',               'Slot/Pocket Width (mm)',    'number', 'mm',        NULL, 10, false, NULL),
    (v_calc_id, 'Total Depth',         'Total Depth of Cut (mm)',   'number', 'mm',        NULL, 11, false, NULL),
    (v_calc_id, 'Depth per Pass',      'Depth per Pass (mm)',       'number', 'mm',        NULL, 12, false, NULL),
    (v_calc_id, 'Cutting Speed',       'Cutting Speed (m/min)',     'number', 'm/min',     NULL, 13, false, NULL),
    (v_calc_id, 'Feed per Tooth',      'Feed/tooth (mm/tooth)',     'number', 'mm/tooth',  NULL, 14, false, NULL),
    (v_calc_id, 'No of Teeth',         'No. of Teeth',              'number', NULL,        NULL, 15, false, NULL),

    (v_calc_id, 'Length of Cut',       'Length of Cut (mm)',        'calculated', 'mm',   NULL, 16, false, NULL),
    (v_calc_id, 'Spindle RPM',         'Spindle RPM',               'calculated', 'RPM',  NULL, 17, false, NULL),
    (v_calc_id, 'No of Depth Passes',  'No. of Depth Passes',       'calculated', NULL,   NULL, 18, false, NULL),
  ...
  -- Fixed geometry constant: 3.7 × π × 10
  UPDATE calculator_fields SET default_value = '3.7 * 3.14159265 * 10'
  WHERE calculator_id = v_calc_id AND field_name = 'Length of Cut';
  ...
  WHERE calculator_id = v_calc_id AND field_name = 'No of Depth Passes';
  ...
  UPDATE calculator_fields SET default_value = '(({Length of Cut} + 10 + {Cutter Diameter}) / ({Feed per Tooth} * {No of Teeth} * {Spindle RPM})) * {No of Depth Passes} * {No of Width Passes}'
  ...
  RAISE NOTICE 'Done — Machining Plunge Milling, ID: %', v_calc_id;
END $$;
