-- ============================================================================
-- Calculator: Machining - Threading
-- Category:   Process
-- Fields:     24 (8 W/C, 5 process, 11 calculated)
-- Formulas:
--   Spindle RPM    = 1000 × Cutting Speed / (π × Threading Diameter)
--   Machining Time = (π × Dia × (Length + 4)) / (1000 × Vc × Pitch) × No of Threading Passes
--   Note: Pitch IS the Feed/rev for threading
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Threading – pitch as feed/rev, spindle RPM, machining time with threading passes, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Threading Diameter',     'Threading Diameter (mm)',    'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Length',                 'Threading Length (mm)',      'number', 'mm',     NULL, 10, false, NULL),
    (v_calc_id, 'Pitch',                  'Thread Pitch (mm)',          'number', 'mm',     NULL, 11, false, NULL),
    (v_calc_id, 'Cutting Speed',          'Cutting Speed (m/min)',      'number', 'm/min',  NULL, 12, false, NULL),
    (v_calc_id, 'No of Threading Passes', 'No. of Threading Passes',   'number', NULL,     NULL, 13, false, '1'),

    (v_calc_id, 'Spindle RPM',            'Spindle RPM',                'calculated', 'RPM', NULL, 14, false, NULL),
    (v_calc_id, 'Machining Time Min',     'Machining Time (min)',       'calculated', 'min', NULL, 15, false, NULL),
    (v_calc_id, 'Machining Time',         'Machining Time (s)',         'calculated', 's',   NULL, 16, false, NULL),
    (v_calc_id, 'Time per Use',           'Time per Use (s)',           'calculated', 's',   NULL, 17, false, NULL),
    (v_calc_id, 'Total Time',             'Total Time (s)',             'calculated', 's',   NULL, 18, false, NULL),
    (v_calc_id, 'Tool Cost per Part',     'Tool Cost/Part (INR)',       'calculated', 'INR', NULL, 19, false, NULL),
    (v_calc_id, 'Machine Cost',           'Machine Cost (INR)',         'calculated', 'INR', NULL, 20, false, NULL),
    (v_calc_id, 'Labour Cost',            'Labour Cost (INR)',          'calculated', 'INR', NULL, 21, false, NULL),
    (v_calc_id, 'Process Cost',           'Process Cost (INR)',         'calculated', 'INR', NULL, 22, false, NULL),
    (v_calc_id, 'Setup Cost',             'Setup Cost (INR)',           'calculated', 'INR', NULL, 23, false, NULL),
    (v_calc_id, 'Total Process Cost',     'Total Process Cost (INR)',   'calculated', 'INR', NULL, 24, false, NULL);

  UPDATE calculator_fields SET default_value = '1000 * {Cutting Speed} / (3.14159265 * {Threading Diameter})'
  WHERE calculator_id = v_calc_id AND field_name = 'Spindle RPM';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Threading Diameter} * ({Length} + 4)) / (1000 * {Cutting Speed} * {Pitch}) * {No of Threading Passes}'
  ...
  RAISE NOTICE 'Done — Machining Threading, ID: %', v_calc_id;
END $$;
