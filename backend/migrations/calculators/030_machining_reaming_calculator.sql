-- ============================================================================
-- Calculator: Machining - Reaming
-- Category:   Process
-- Fields:     24 (8 W/C, 4 process, 12 calculated)
-- Formulas:
--   Spindle RPM       = 1000 × Cutting Speed / (π × Diameter)
--   Drill Bit Const C = (Diameter / 2) × 0.577  [drill tip approach]
--   Machining Time    = (π × Dia × (Length + 4 + C)) / (1000 × Vc × Feed/rev)
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Reaming – spindle RPM, drill tip constant C, machining time with tip approach, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Diameter',           'Reaming Diameter (mm)',      'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Length',             'Reaming Length (mm)',        'number', 'mm',     NULL, 10, false, NULL),
    ...
    (v_calc_id, 'Drill Bit Constant', 'Drill Bit Constant C (mm)',  'calculated', 'mm',  NULL, 14, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '({Diameter} / 2) * 0.577'
  WHERE calculator_id = v_calc_id AND field_name = 'Drill Bit Constant';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Diameter} * ({Length} + 4 + {Drill Bit Constant})) / (1000 * {Cutting Speed} * {Feed per Rev})'
  ...
  RAISE NOTICE 'Done — Machining Reaming, ID: %', v_calc_id;
END $$;
