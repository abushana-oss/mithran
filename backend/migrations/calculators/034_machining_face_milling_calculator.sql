-- ============================================================================
-- Calculator: Machining - Face Milling
-- Category:   Process
-- Fields:     28 (8 W/C, 8 process, 12 calculated)
-- Formulas:
--   Spindle RPM    = 1000 × Cutting Speed / (π × Cutter Diameter)
--   No of Passes   = (Width − Cutter Dia/2) / (Cutter Dia − 2)   [lateral coverage]
--   Machining Time = ((Length + 10 + Cutter Dia/2) / (Feed/tooth × No Teeth × RPM))
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Face milling – cutter RPM, lateral passes for full width, depth passes, machining time, full cost breakdown'
  )
  ...
  UPDATE calculator_fields SET default_value = '(({Length} + 10 + {Cutter Diameter} / 2) / ({Feed per Tooth} * {No of Teeth} * {Spindle RPM})) * {No of Passes} * ({Total Depth} / {Depth per Pass})'
  ...
  RAISE NOTICE 'Done — Machining Face Milling, ID: %', v_calc_id;
END $$;
