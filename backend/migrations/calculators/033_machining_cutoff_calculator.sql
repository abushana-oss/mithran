-- ============================================================================
-- Calculator: Machining - Cutoff
-- Category:   Process
-- Fields:     23 (8 W/C, 4 process, 11 calculated)
-- Formulas:
--   Spindle RPM    = 1000 × Cutting Speed / (π × Workpiece Diameter)
--   Machining Time = (π × Dia × (Length of Travel + 10)) / (1000 × Vc × Feed per Tooth)
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Cutoff/parting – spindle RPM, machining time for parting blade travel, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Length of Travel',   'Length of Travel (mm)',     'number', 'mm',        NULL, 10, false, NULL),
    (v_calc_id, 'Cutting Speed',      'Cutting Speed (m/min)',     'number', 'm/min',     NULL, 11, false, NULL),
    (v_calc_id, 'Feed per Tooth',     'Feed/tooth (mm/tooth)',     'number', 'mm/tooth',  NULL, 12, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '(3.14159265 * {Workpiece Diameter} * ({Length of Travel} + 10)) / (1000 * {Cutting Speed} * {Feed per Tooth})'
  ...
  RAISE NOTICE 'Done — Machining Cutoff, ID: %', v_calc_id;
END $$;
