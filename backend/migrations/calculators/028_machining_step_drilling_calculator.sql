...
BEGIN
  INSERT INTO calculators (
    ...
    v_user_id,
    'Machining - Step Drilling',
    'process',
    'single',
    false,
    false,
    'Step drilling – spindle RPM, machining time with depth approach, full cost breakdown'
  )
  ...
  VALUES
    ...
    (v_calc_id, 'Hole Diameter',      'Hole Diameter (mm)',      'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Depth',              'Drilling Depth (mm)',     'number', 'mm',     NULL, 10, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '1000 * {Cutting Speed} / (3.14159265 * {Hole Diameter})'
  WHERE calculator_id = v_calc_id AND field_name = 'Spindle RPM';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Hole Diameter} * ({Depth} + 10)) / (1000 * {Cutting Speed} * {Feed per Rev})'
  ...
  RAISE NOTICE 'Done — Machining Step Drilling, ID: %', v_calc_id;
END $$;
