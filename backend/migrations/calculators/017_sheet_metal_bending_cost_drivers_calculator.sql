-- ============================================================================
-- Calculator: Sheet Metal - Bending Cost Drivers
-- Category:   Process
-- Fields:     22 (8 Number inputs from linked calculators, 7 Rate inputs, 6 Calculated, 1 Number)
-- Standard cost driver formulas (no No Of Impressions division for bending):
--   Machine Cost = (MHR/60) * (Cycle Time/60)
--   Labor Cost   = (DLR/60) * Direct Labors * (Cycle Time/60)
-- Corrected Yield Cost formula:
--   (1 - Yield%) * (NMC + Machine + Labor + Setup + Inspection - Net Weight/1000 * Scrap Price)
-- Reusable across all processes that include Bending as a secondary operation.
...
BEGIN
  INSERT INTO calculators (
    ...
    v_user_id,
    'Sheet Metal - Bending Cost Drivers',
    'process',
    'single',
    false,
    false,
    'Bending cost driver calculations – machine, setup, labor, inspection, yield, net process cost'
  )
  ...
  VALUES
    ...
    -- Inputs from Bending Manufacturing calculator
    ...
    (v_calc_id, 'Direct Labors',       '# of Direct Labors',             'number', NULL,   NULL,  6, false, '1'),
    ...
    (v_calc_id, 'Machine Hour Rate',   'Machine hour Rate ($)',           'number', '$/hr', NULL, 15, false, '15'),
  ...
  UPDATE calculator_fields SET default_value =
    '({Machine Hour Rate} / 60) * ({Cycle Time} / 60)'
  ...
  UPDATE calculator_fields SET default_value =
    '({Direct Labor Rate} / 60) * {Direct Labors} * ({Cycle Time} / 60)'
  ...
  RAISE NOTICE 'Done — Sheet Metal Bending Cost Drivers, ID: %', v_calc_id;
END $$;
