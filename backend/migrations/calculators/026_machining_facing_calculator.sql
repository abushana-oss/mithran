-- ============================================================================
-- Calculator: Machining - Facing
-- Category:   Process
-- Fields:     26 (8 W/C inputs, 6 process inputs, 12 calculated)
-- Formulas:
--   Spindle RPM      = 1000 × Cutting Speed / (π × Workpiece Diameter)
--   No of Passes     = Total Depth of Cut / Depth per Cut
--   Machining Time   = (π × Dia × (Length + 10)) / (1000 × Vc × Feed/rev) × Passes
--   Machining Time s = Machining Time Min × 60
--   Time per Use     = CEIL(Machining Time)
--   Total Time       = No of Uses × Time per Use
--   Machine Cost     = MHR per Hour × Total Time / 3600
--   Labour Cost      = LHR per Hour × Total Time / (3600 × OLE/100)
--   Process Cost     = Machine Cost + Labour Cost
--   Setup Cost       = Process Cost × Setup Percentage / 100
--   Total Process Cost = Process Cost + Setup Cost + Tool Cost per Part
...
BEGIN
  INSERT INTO calculators (
    ...
    false,
    false,
    'Facing operation – spindle RPM, no. of passes, machining time, full cost breakdown'
  )
  ...
  VALUES
    ...
    -- Process inputs
    (v_calc_id, 'Workpiece Diameter', 'Workpiece Diameter (mm)',  'number', 'mm',     NULL,  9, false, NULL),
    (v_calc_id, 'Length of Cut',      'Length of Cut (mm)',       'number', 'mm',     NULL, 10, false, NULL),
    (v_calc_id, 'Total Depth of Cut', 'Total Depth of Cut (mm)',  'number', 'mm',     NULL, 11, false, NULL),
    (v_calc_id, 'Depth per Cut',      'Depth per Cut (mm)',       'number', 'mm',     NULL, 12, false, NULL),
    (v_calc_id, 'Cutting Speed',      'Cutting Speed (m/min)',    'number', 'm/min',  NULL, 13, false, NULL),
    (v_calc_id, 'Feed per Rev',       'Feed/rev (mm/rev)',        'number', 'mm/rev', NULL, 14, false, NULL),

    -- Calculated
    (v_calc_id, 'Spindle RPM',        'Spindle RPM',              'calculated', 'RPM', NULL, 15, false, NULL),
    (v_calc_id, 'No of Passes',       'No. of Passes',            'calculated', NULL,  NULL, 16, false, NULL),
    (v_calc_id, 'Machining Time Min', 'Machining Time (min)',     'calculated', 'min', NULL, 17, false, NULL),
    (v_calc_id, 'Machining Time',     'Machining Time (s)',       'calculated', 's',   NULL, 18, false, NULL),
  ...
  UPDATE calculator_fields SET default_value = '1000 * {Cutting Speed} / (3.14159265 * {Workpiece Diameter})'
  WHERE calculator_id = v_calc_id AND field_name = 'Spindle RPM';

  UPDATE calculator_fields SET default_value = '{Total Depth of Cut} / {Depth per Cut}'
  WHERE calculator_id = v_calc_id AND field_name = 'No of Passes';

  UPDATE calculator_fields SET default_value = '(3.14159265 * {Workpiece Diameter} * ({Length of Cut} + 10)) / (1000 * {Cutting Speed} * {Feed per Rev}) * {No of Passes}'
  ...
  RAISE NOTICE 'Done — Machining Facing, ID: %', v_calc_id;
END $$;
