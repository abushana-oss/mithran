    ...
    try {
      ...
      if (inspection.selected_items && Array.isArray(inspection.selected_items)) {
        const qualityApprovedItems = inspection.selected_items.map((item: any) => ({
          bom_item_id: item.bomItemId || item.id,
          approved_quantity: item.quantity || 1,
          ...
          approved_by: userId,
          approved_at: new Date().toISOString(),
          qc_certificate_number: null, // Will be auto-generated by database trigger
          quality_grade: 'A', // Default grade - can be customized later
          created_by: userId,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        }));
