...
let queryBuilder = this.supabaseService
  ...
  async getBulkTotalCosts(
    ...
    if (error) {
      this.logger.error(`Error fetching bulk packaging/logistics costs: ${error.message}`, 'PackagingLogisticsCostService');
      throw new InternalServerErrorException(`Failed to fetch bulk packaging/logistics costs: ${error.message}`);
    }
